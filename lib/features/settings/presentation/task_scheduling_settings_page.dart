      ...
      children: [
        ...
        ...buildSettingsTileGroup([
          SettingsStepperTile(
            title: '最大并发任务数',
            subtitle:
                '统一限制首页加载、搜索链接验证、后台元数据、Emby 刷新及 NAS / WebDAV 扫描补全的并发规模。${isTelevision ? 'TV 推荐 2，性能较好的设备可设为 3。' : ''}',
        ...
        ...buildSettingsTileGroup([
          SettingsStepperTile(
            title: '首批元数据预取数量',
            subtitle:
                'Hero、评分和各首页分区合计先处理的数量；剩余内容会在后台分批补齐，不会丢弃。${isTelevision ? 'TV 推荐 12。' : ''}',
          ...
          SettingsStepperTile(
            title: '交互结束后恢复时间',
            subtitle:
                '滚动、焦点移动或页面切换停止后，等待多久再继续后台补全。${isTelevision ? 'TV 推荐 400 毫秒。' : ''}',
        ...
        ...buildSettingsTileGroup([
          SettingsStepperTile(
            title: '首页首批优先模块数',
            subtitle:
                '进入首页后优先启动的前几个模块；其余模块会自动错峰加载，不会遗漏。${isTelevision ? 'TV 推荐 1–2。' : ''}',
