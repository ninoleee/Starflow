...
- 详情页不再在进入时自动搜索字幕；只有已经拿到可播放资源并手动点击资源信息区里的“搜索字幕 / 刷新字幕”时，才会按 `设置 -> 播放 -> 字幕` 里的配置手动搜索。结构化链当前支持 `ASSRT API / OpenSubtitles / SubDL`，并只展示最多 `10` 条可直接挂载的字幕；`ASSRT` 需要在设置里填写 Token 后才会启用
...
- 播放器页与详情页现在复用同一条手动触发的应用内字幕搜索/下载链路：
  ...
  - 结构化链会先在应用内预下载、校验并筛掉不可直接挂载的结果；只有可直接加载的字幕才会展示
  - 结构化链验证通过后的字幕会缓存到临时目录；后续再次选择同一结果时都会优先复用，不重复下载
