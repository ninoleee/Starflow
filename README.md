...
- 设置页已加入“高性能模式”开关，所有客户端开启后都会关闭磨砂/模糊背景、菜单栏自动隐藏、Hero 全屏背景图与通用阴影，把首页 Hero 固定为静态单卡海报样式、导航切换改成静态、详情页大图区进一步瘦身，并继续偏向硬解与轻量字幕/叠层；已单独列出的轻量化子项之后仍可按需单独改回
...
- 首页内容模块与后台元数据预取已拆成两套独立调度预算：默认最大并发均为 `2`，首页首批优先模块默认 `2`，元数据首批预取默认 `12`；剩余任务会错峰继续完成，不会因为首批数量而被丢弃
- 冷启动首页刷新由 Bootstrap 与主壳共享完成标记，Bootstrap 已执行时主壳会跳过重复刷新；启动首页刷新与同时刷新 Emby 的开关已集中到“高性能模式”页面并独立持久化
...
- 手动索引管理在 `TV` 模式下会为自动更新、搜索和 `WMDB / TMDB` 应用按钮保留稳定焦点节点；遥控器按上 / 下时会按固定焦点链强制切换，并自动跳过当前不可用的按钮
...
- 高性能模式开启后，所有客户端都会：
  ...
  - 关闭首页 `Hero` 与列表卡片的运行时局部 overlay 更新；条目只在应用启动、保存设置或显式刷新后再应用新的缓存信息
...
- 可切换透明磨砂效果，降低弱性能设备上的视觉开销
  - 高性能模式开启时会先套用“关闭”推荐值，但之后仍可单独重新打开
- 可配置“自动隐藏菜单栏”
...
- 可配置“运行时卡片 / Hero 局部更新”
  ...
  - 高性能模式开启时会先套用“关闭”推荐值，但之后仍可单独重新打开
  - `TV` 端仍会强制关闭，避免焦点浏览和滚动过程中被后台局部刷新打断
...
- 详情页信息管理页在 `TV` 模式下会接管上 / 下方向键，按“自动更新 -> 搜索词 -> 年份 -> 剧集优先 -> 开始搜索 -> 结果应用按钮”的顺序强制切换焦点
- 搜索页支持最近搜索词和来源记忆
