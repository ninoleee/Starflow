...
- 应用侧 trace / log 保持静音，内置 `MPV` 日志级别压到 `error`
...
- 手动索引管理在 `TV` 模式下会为自动更新、搜索和 `WMDB / TMDB` 应用按钮保留稳定焦点节点；搜索或更新导致结果区异步重建后，如果当前没有可用焦点，会优先回到可应用结果，再回退到“开始搜索”等固定入口
...
- 详情页信息管理页会在搜索、自动更新和应用结果状态变化后做焦点兜底，避免异步重建后遥控器落到无焦点状态
- 搜索页支持最近搜索词和来源记忆
