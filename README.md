...
- 应用侧 trace / log 输出当前已统一静音；保留的 trace 开关仅用于兼容已有设置字段，不再产生运行时输出，内置 `MPV` 日志级别也已压到 `error`
...
- 详情页不再在进入时自动搜索字幕；只有已经拿到可播放资源并手动点击资源信息区里的“搜索字幕 / 刷新字幕”时，才会按 `设置 -> 播放 -> 字幕` 里的配置手动搜索。结构化链当前支持 `ASSRT API / OpenSubtitles / SubDL`，并只展示最多 `10` 条可直接挂载的字幕；`ASSRT` 未填写 Token 时会直接走网页搜索，填写 Token 后则优先走官方 API，并可按设置决定在 API 没有可用结果时是否回退网页链路
...
- Windows 播放链路仍保留对应 trace 字段以兼容已有设置，但应用侧 `playback / subtitle / metadata` trace 当前都已静音，不再产生运行时输出
...
- 播放器页与详情页现在复用同一条手动触发的应用内字幕搜索/下载链路：
  - 在线字幕来源由设置统一控制；当前支持 `ASSRT / OpenSubtitles / SubDL`
  - 新链路会优先利用文件路径、文件哈希、`IMDb ID / TMDB ID`、季集号、年份和标题生成结构化查询，并在应用内先下载校验；只有可直接加载的 `SRT / ASS / SSA / VTT` 或可解压 `ZIP` 结果才会展示给用户
  - `ASSRT` 在设置页里和其他来源放在同一组；启用后可填写 `Token`，填写后优先走官方 API，未填写则继续走网页搜索；当 `Token` 已填写时，还可单独控制是否允许网页回退
  - `OpenSubtitles` 需要在设置页填写账号密码，并通过 `--dart-define=STARFLOW_OPENSUBTITLES_API_KEY=...` 注入 API Key；`SubDL` 需要在设置页填写 API Key
  - `ASSRT` 网页查询在站点命中失败时，会继续按更短的片名做 fallback
  - `ASSRT` 如果返回站点错误页，会直接按来源失败上报，不再伪装成“0 结果”
  - 结构化链验证通过后的字幕会缓存到临时目录，网页链路下载结果会继续缓存到本地；后续再次选择同一结果时都会优先复用，不重复下载
...
- 播放设置
  ...
  - 字幕
    - 默认字幕策略
    - 字幕大小
    - 在线字幕来源（统一配置 `ASSRT / OpenSubtitles / SubDL`，其中 `ASSRT` 可选官方 API + 网页回退）
- 首页模块设置
