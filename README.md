...
- 扫描时支持：
  ...
  - 同一影片目录下如果同时存在至少两个 `1080P / 4K / 国英双语 / 英语 / 高码 / 字幕` 等技术版本目录，且文件没有季集信号，会合并为一个电影卡片；版本目录下再套 `Disc / CD / Remux` 等组织层时，其下所有可播放文件也会归入同一组，详情页仍保留所有可播放版本，不再把技术目录建成剧集
...
- 可配置后台播放：
  ...
  - iOS 开启后切后台时会继续后台播放音频，锁屏 / 控制中心优先显示当前条目的海报，缺海报时回退到背景图
  - iOS 的音频会话配置统一走同一入口；配置失败会写入结构化 native 日志，便于排查后台播放被系统拒绝的问题
