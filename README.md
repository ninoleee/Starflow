...
- 设置页已加入“高性能模式”开关，所有客户端开启后都会关闭磨砂/模糊背景、压低导航切换/启动页/首页 Hero 等动画，并简化播放器叠层；`TV` 端还会进一步简化焦点高亮，优先保证老盒子流畅度
...
- 高性能模式开启后，所有客户端都会：
  - 关闭磨砂/模糊背景与通用面板阴影
  - 压低启动页过渡、主导航切换、首页 Hero 分页与翻页按钮缓动
  - 更积极偏向硬解，并简化播放器叠层与字幕背景
- 内置 `MPV` 当前还额外做了启动期性能调优：
  - 按本地 / 远程 / 重码率片源动态调整 `bufferSize`
  - 远程流会提高预读与缓存等待参数，降低刚进播放时的抖动
  - 高性能模式、重片源或高压力场景下会尝试应用更激进的 `MPV fast profile`
