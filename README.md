...
- 支持应用内直接播放资源
- 支持详情页按需补全元数据与评分：`TMDB` 补影片信息，`IMDb` 补评分
...
- 详情页支持显示 `IMDb / 豆瓣` 等评分标签
- 缺少海报、简介、导演、演员时，可按设置自动用 `TMDB` 补全
- 缺少 `IMDb` 评分时，可按设置自动补一个 `IMDb x.x` 标签
- 如果是剧集，支持按季切换
- 每一季的剧集使用横向滑动轨道展示
...
- `TMDB` 负责补海报、简介、导演、演员、类型、时长这类影片信息
- 只在详情页按需触发，不会扫描首页和媒体库
- 需要先填写 `TMDB Read Access Token` 才会生效
...
- `IMDb` 只负责补评分，不负责补海报和简介
- 当前通过 IMDb 官方公开评分数据集补分
- 第一次命中某个条目时可能会比普通详情页稍慢一点，后续会复用已拿到的结果
...
- `TMDB` 自动补全需要你自己提供 `Read Access Token`
- `IMDb` 当前只补评分，不补完整影片信息
