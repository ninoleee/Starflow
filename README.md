...
- Windows 的内置 `MPV` 在打开新会话前会串行等待上一实例完成 `pause -> stop -> dispose`，并补了播放 trace 与销毁保护，减少叠音、全屏退出和悬浮控件销毁阶段的异常
...
- 像 `十三邀第九季` 这类“标题本体 + 季后缀”的目录，如果上层没有形成这种同级季结构，就会继续保留目录自身标题，不再直接回退到集合根或来源目录名
...
- Windows 的内置 `MPV` 会在下一次打开前串行等待上一实例完成 `pause -> stop -> dispose`，减少快速切换片源或退出全屏时出现叠音
- Windows 播放链路新增了 `lib/core/utils/playback_trace.dart` 调试 trace，打开、缓冲、全屏切换和叠层显隐都能输出 scoped 日志，便于排查桌面端问题
