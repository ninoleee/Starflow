...
- 可配置后台播放：
  ...
  - iOS 开启后切后台时会继续后台播放音频；内置 `MPV` 会关闭 `media_kit_video` 默认的切后台自动暂停，锁屏 / 控制中心优先显示当前条目的海报，缺海报时回退到背景图
  - iOS 的音频会话配置统一走同一入口，并按后台播放、Now Playing、原生播放器记录持有者；只有最后一个真实持有者释放后才停用共享会话。配置失败会写入结构化 native 日志，便于排查后台播放被系统拒绝的问题
  - iOS 原生播放器关闭系统自动 Now Playing 发布，只保留 Starflow 持续同步的单一发布方，避免系统元数据覆盖鉴权海报；App 前后台事件仅保存进度，不切换或重建 Now Playing
