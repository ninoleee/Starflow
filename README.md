...
- 首页内容模块与后台元数据预取已拆成两套独立调度预算：默认最大并发均为 `2`，首页首批优先模块默认 `2`，元数据首批预取默认 `12`；首页后续批次默认间隔 `350ms`，元数据后续批次默认间隔 `300ms`，剩余任务都会继续完成
...
- 详情页“资源信息 / 播放版本”这组高频变化区已经改成局部 `ValueListenableBuilder`，资源匹配进度和播放版本切换不再带动整页重建
...
- 性能与后台设置
  ...
  - NAS / WebDAV 来源、集合扫描与索引补全并发：均可在 `1–4` 之间调整，默认分别为 `1 / 2 / 2`
  - 界面、Hero、播放和调度选项各自持久化，不存在批量改写其它子项的总开关
  - 旧版高性能模式标记只用于兼容迁移：遗留的 `1 / 6 / 1 / 1` 最小调度组合会在升级时迁移为 `2 / 12 / 2 / 2`；新版不再保存该标记，用户主动设置的最小值会继续保留
- 日志管理
...
- `SharedPreferences`
  - 应用设置
  - 详情缓存
    - 当前会缓存详情目标、本地资源匹配候选列表和当前“播放版本”选择
