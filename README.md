...
- 支持排除目录关键字
- 支持增量更新和强制重建索引
- 增量更新默认保留未变化 WebDAV 子目录的持久化快照，只重新检查作用域根目录以发现新增或删除；新增文件才建立索引并补元数据，已有条目直接复用，不会越过到其他分区。夸克保存后有新文件时仍会显式失效对应来源快照，确保新内容立即进入索引
...
- 播放设置
  ...
  - 三个页面全部采用即时自动保存，页面不再显示“取消 / 确定 / 保存 / 完成”提交按钮；连续输入会短暂合并写入，返回页面前会等待最后一次修改落盘
  - 播放
    - 播放器内核
    - 解码模式
    - 最大打开超时
