...
核对日期：2026-09-24。本文记录当前工作区的焦点边界与本轮修复，不代表真实遥控器／输入法验收通过。组件职责以 [架构说明](architecture.md) 为准，网络影响见 [开发网络](development-network.md)。
...
2026-09-24 首页 Hero 下移回归：`test/tv_focus_regressions_test.dart` 与 `test/features/home/presentation/home_page_presentation_test.dart` 共 60 项 Flutter 测试通过，定向静态分析通过。覆盖横向列表滚到第 15 张后回 Hero 再下移、空模块跨段寻焦、焦点卡片删除、Hero 重排、首页返回恢复和自动轮播。这是主机 widget 验证，不代表真实遥控器验收。
...
验证结果与播放生命周期修复合并记于 [播放器流畅度审查的十方向复审补充](reviews/player-smoothness-review-2026-09-20.md#十方向复审补充2026-09-20)。上方既有批次仍保留各自历史时间与范围。
