...
- 直播返回键在当前路由消费完整的 KeyDown / KeyRepeat / KeyUp 周期：菜单在 KeyDown 关闭，无菜单则等待匹配的 KeyUp 才退出播放器，进入后台取消待松键意图。播放器不能先退场后让松键落到主界面，再触发一次系统返回。`live_tv_page_test.dart` 使用 GoRouter 宿主检查三类叠层的按键处理结果、焦点恢复和播放会话保留；`live_navigation_resume_test.dart` 保留菜单栏打开的真实播放器，mock Android 平台视图通道，检查长按与系统返回只回频道页、不弹退出 App 确认。这是主机组件回归，真实遥控器仍需复验。
...
- `LivePlayerPage` 初始焦点落在 `live-player` 画布。仅当它持有主焦点、且未打开频道／节目单／设置时，上下/频道键才循环换台，长按允许重复输入，由播放控制器的 180ms 窗口合并开流；确认/左/右/菜单只在 KeyDown 执行打开命令。确认打开频道、左打开频道/分组、右打开节目单，菜单打开播放设置并帧末把焦点移到 `live-settings-close`。
