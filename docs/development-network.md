...
- 网络同步通过统一 `StarflowHttpClient` 访问用户配置的 WebDAV：连接测试使用 `PROPFIND Depth: 0`，上传逐级 `MKCOL` 创建相对目录，再 `GET` / 条件 `PUT` 更新 `starflow-sync.json`，下载使用 `GET`。基础目录需预先存在，测试不会创建目录。请求不跟随重定向，用户需填写最终地址；采用可选 Basic 认证，建议 HTTPS。
- 同步请求包含响应读取最多等待 30 秒，单个文件上限 16 MB；认证、格式、缺失内容和 ETag 冲突均显示失败，不将服务端响应正文放进错误消息。没有周期性网络任务。Web 端受 CORS 限制，服务器或开发代理须允许 `PROPFIND / MKCOL / GET / PUT`、认证及条件请求头，并暴露 ETag 才能进行版本冲突检查。
...
- 115 目录管理和独立同步删除使用 `/files` 分页读取及 `https://webapi.115.com/rb/delete` 表单 POST（`pid`、`fid[0]` 等字段），不访问永久清空回收站接口，不自动重试写操作。同步删除只在 WebDAV 成功后执行 115 删除，403/405 不绕过；超时需确认远端状态。115 监听目录使用精确来源 ID 与当前 URI，地址迁移后需重新选目录。删除不触发 SmartStrm 生成任务，也不声称清理远端生成的 STRM；实际账号接口仍需联调。
- 115 分享转存使用独立 Cookie，通过共享 HTTP 客户端访问 `https://webapi.115.com/files`（分页目录）、`/share/snap`（分页分享内容）及 `/share/receive`（表单 POST 转存）。Cookie 只发送到固定 API 域名，不访问分享文本中的任意主机；接收码随分享请求发送。请求超时为 30 秒，转存写操作不自动重试，超时需先确认远端结果。保存成功后，若公共 SmartStrm Webhook 和独立 `cloud115SmartStrmTaskName` 均已配置，会通过既有客户端发送 `a_task` JSON，使用 115 任务名、公共延迟（最少 1 秒）及 115 保存路径作为 `task.storage_path`，根目录省略此字段；不向 Webhook 发送网盘 Cookie，不回退到夸克任务名。测试按钮分别位于夸克和 115 网盘设置页，使用该页当前任务名和保存路径，共用 Webhook 与延迟；公共 SmartStrm 页无测试按钮。触发失败仍继续媒体源刷新，不轮询远端 STRM 完成状态。115 暂不接入直链播放和夸克同步删除；测试使用模拟响应，真实账号兼容性需账号联调确认。
...
- MPV / Exo 的共享可靠性配置位于 `config/playback_policy.json`；修改后运行 `dart tool/generate_playback_policy.dart`，通过 `dart tool/generate_playback_policy.dart --check` 验证。生成不联网、不升级依赖。两端自动卡顿重建最多两次，MPV 运行期错误重建也使用同一预算；切集/手动重试重置，实例释放不重置。Exo 缓冲增长仍可延长启动观察，但单轮启动上限 `120s`，MPV 自适应期限也以此封顶。`playback.reliability` 在状态/恢复边界记录 engine、policyVersion、phase、positionMs，恢复记录 action、reason、attempt，不新增预检、测速或远端上报。
