...
- iOS 锁屏 / 控制中心封面只复用播放目标里已有的 `posterUrl / backdropUrl` 与对应 headers；系统 Now Playing 首次展示封面时可能发起一次图片加载，不新增元数据来源或播放预检请求
- iOS 后台播放的音频会话配置已收口到 native helper；失败只写结构化 native 日志，不触发额外重试或业务请求
...
- `starflow_http_client.dart`：共享传输客户端、Web 代理适配、`20` 秒响应头超时与脱敏请求日志
...
- 同一电影根目录下的多个技术版本目录会在本地结构推断中收口为一个电影和多个播放版本；该判断要求至少两个同级版本目录且无季集信号，版本目录下的嵌套文件也沿用同一影片根目录，不新增远程请求。旧误分类通过条目级指纹在下次增量刷新中定向重建，未命中的条目继续复用原索引
- 电影多版本详情页在 Hero 下方直接展示“播放版本”选择控件；“本地资源”来源切换与当前来源内的播放版本选择分别维护，WMDB / TMDB 只用影片根目录名匹配，文件名中的清晰度、编码和发布组信息不会再作为电影标题提交（明确匹配的 NFO / sidecar 仍优先），版本展开和选择均复用本地索引，不新增远程请求
...
- 独立字幕搜索页与播放器页的在线字幕搜索现在都走应用内实现，不依赖外部浏览器；是否真的发起请求由 `设置 -> 播放 -> 播放器与字幕 -> 字幕` 里的在线字幕配置控制，当前来源统一为 `ASSRT / OpenSubtitles / SubDL`
...
- `设置 -> 数据与维护 -> 日志` 可以控制本地日志开关、最大容量和记录/展示级别；默认容量为 `20 MB`
...
- `设置 -> 内容与来源 -> 搜索服务管理 -> 搜索来源` 会直接限制搜索页实际参与执行的本地媒体源与在线搜索服务
...
- 在线字幕来源由 `设置 -> 播放 -> 播放器与字幕 -> 字幕` 统一控制；当前支持 `ASSRT / OpenSubtitles / SubDL`
- 字幕设置里的“优先语言”当前提供 `简体中文 / 繁体中文 / 英语 / 日语 / 韩语` 这些常见选项；不选时会按字幕结果和系统语言自动处理
