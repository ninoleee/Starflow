...
- 季目录确认现在要求同级里存在同一种命名方式的多个兄弟目录，例如 `Season 1 / Season 2`、`第1季 / 第2季`、`2.巴以 / 5.美国 / 9.韩国`；这一步完全发生在本地结构推断阶段，不会为了重新判季额外发网络请求
- 一旦同级确认这一层是季目录，就会把上一级当剧名；像 `怪奇物语/Season 1/Season 2` 会把 `怪奇物语` 作为剧名，这同样只是本地结构层级判断，不新增网络请求
- 同级多个“同剧名 stem + 季号 + 发布信息”的发布目录，也会在本地结构推断里确认成季层，例如 `怪奇物语/Stranger.Things.S02.2160p.BluRay.REMUX`、`怪奇物语/Stranger.Things.S05.2160p.BluRay.REMUX`；季号直接取目录里识别出的 `2 / 5`，不会因为扫描顺序重排，也不新增网络请求
- `十三邀第九季` 这类“标题本体 + 季后缀”的目录，如果上层没有形成这种同级季结构，就会继续保留目录自身标题，不会因为目录名里带季字样就直接回退到集合根
