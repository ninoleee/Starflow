...
- 同一部剧下像 `2.巴以 / 5.美国 / 9.韩国` 这类数字子目录的季归属修正也完全发生在本地结构推断阶段，不会为了重新判季额外发网络请求
- 如果路径里存在显式季目录，例如 `Season 1 / 第1季 / 5.美国`，现在即使只有一季也会继续保留季节点；这也是本地结构层级判断，不新增网络请求
...
- 非 `TV` 内嵌 `MPV` 改为优先使用 `media_kit` 自带控制条，也只是本地播放器 UI 收敛；控制条里额外保留的 `1` 个扩展按钮仅用于打开应用内字幕 / 音轨 / 外挂字幕 / 在线字幕 / 字幕偏移面板，不新增新的服务端接口
- 非 `TV` 内嵌 `MPV` 顶部返回按钮左移到更贴近原生播放器左上角的位置，也只是本地控件主题 margin 调整，不涉及新的网络协议
