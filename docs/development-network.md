...
- 剧集会话的字幕选择记忆只保存本地轨道特征；MPV、Android ExoPlayer 和 iOS AVPlayer 切集后在新一集已有轨道中重新匹配，双字幕分别匹配主、副轨道，不请求服务器查询字幕，也不复用上一集的轨道 ID。外挂和在线下载字幕文件仍限定当前单集
...
- 播放开始时的自动选轨不增加网络请求：按优先语言匹配、片源 Forced 标记、片源 Default 标记的顺序在本地完成；“默认关闭”会跳过自动选轨
