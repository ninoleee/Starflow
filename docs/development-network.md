...
- 同一电影根目录下的多个技术版本目录会在本地结构推断中收口为一个电影和多个播放版本；该判断要求至少两个同级版本目录且无季集信号，清晰度、语言（包括英语、日语、韩语）、字幕或码率等组合均可作为技术标签，版本目录下的嵌套文件也沿用同一影片根目录，不新增远程请求。旧误分类通过条目级指纹在下次增量刷新中定向重建，未命中的条目继续复用原索引
...
- 用户在媒体库执行“增量更新”时会先按所选来源清除持久化 WebDAV 目录快照，再重新列出作用域内目录；这是为了发现服务端未更新父目录指纹时新增的文件，后续索引与元数据补全仍只处理变化条目
