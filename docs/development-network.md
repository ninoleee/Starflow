...
- `TV` 详情页信息管理页新增的稳定焦点节点和上 / 下方向键强制切换，只影响遥控器焦点移动，不改变 `WMDB / TMDB` 搜索或写回接口
...
- 设置页里已单独列出的轻量化子项只是会先被高性能模式套用推荐值，之后仍可单独改回；这同样只是本地 UI 联动，不增加新的网络请求
...
- `设置 -> 日志` 可以控制本地日志开关、最大容量和记录/展示级别；默认容量为 `20 MB`
...
- 结构化验证缓存会额外写入 `starflow/validated_online_subtitles`；这些字幕缓存现在也会出现在“设置 -> 本地存储”里并支持手动清理
