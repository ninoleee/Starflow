...
- WebDAV 结构推断将已确定的母目录固定为剧名；`#004 长标题/同名视频.mp4` 这类编号单集目录只提供第 1 季集号和单集标题，不单独成为剧名或季。规则变化通过 `webdav-v12` 触发本地索引重建，不新增远端请求类型
...
- 夸克一键保存会在转存、名称修正、SmartStrm 触发和媒体源刷新安排时实时替换页面提示。依赖真实文件路径的前三段严格按顺序等待；刷新只安排后台任务，因此不会为了等待索引完成而延迟保存结果
