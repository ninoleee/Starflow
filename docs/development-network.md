...
- 配置夸克 Cookie 时，每条去重后的在线夸克结果会额外请求一次分享 token 接口做轻量有效性校验；并发复用设置中的“最大并发任务数”，单条最多等待 `8s`。搜索任务和校验队列全部完成后才一次性提交结果；明确死链不会进入列表，超时、握手失败或限流不会触发重试风暴，也不会误丢结果；用户保存时仍重新获取 token 和文件列表完成最终校验
