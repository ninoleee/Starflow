...
- 媒体库只保留“增量更新”和“重建索引”入口。手动增量保持原先按来源清除持久化 WebDAV 子目录快照、遍历所选分区的行为，扫描上限、索引复用、删除判定和并发预算不变；不引入目录级缓存迁移、持久化刷新队列或后台轮换补扫
...
- 当前作用域版本为 `NasMediaIndexer._webDavMetadataSchemaVersion = webdav-v15`；版本变化会使旧作用域重新索引。母目录带有明确剧集证据时，画质子目录继续归入同一系列而不是电影版本；单资源结构指纹仍独立记录最终 `itemType / season / episode`，供重建索引和诊断比较使用；作用域未失效的普通增量不重新分类已有记录，两种机制不可混淆
...
- `NasMediaRecognizer` 解析父目录单集提示前移除 `E01-16 / EP01-16 / E01-E16` 等递增范围，避免合集目录将全部子文件赋为第 1 集；范围中的 `S01` 仍可提供季号。结构推断的组内数字前缀允许紧接括号说明，保留 `16(1080x264).(mp4).strm` 的真实集号。该分类变更使用 `webdav-v15`，后续索引刷新会重新识别旧记录；真实目录回归覆盖《局部》三季的 `16 / 16 / 12` 个独立文件。
- sidecar 读取
- `streamdetails`
- 外部 ID 提取
...
- 演职员头像可跳转到人物关联影片页，`TV` 焦点动作只包裹圆形头像并采用圆形焦点框与 `1.06` 缩放，姓名在头像下方但不参与焦点区域。人物作品列表继续复用首页同款海报卡片；卡片右上角会优先显示题材/类型标签，左下角继续显示可用评分标签
