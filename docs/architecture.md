...
- 统一路径规则的作用域版本为 `webdav-v12`；版本变化会使旧作用域重新索引。单资源结构指纹仍独立记录最终 `itemType / season / episode`，供重建索引和诊断比较使用；增量阶段不再用它重新分类已有记录，两种失效机制不可互相替代
...
- `SE08.06` 这类文件名会同时提供季号和集号；结构推断确定母目录为剧集根后，母目录名是唯一剧名，子目录/文件只提供季集与单集展示信息。标题中间的 `#19` 和子目录开头的 `#004` 都可作为第 1 季集号，子目录不能另起剧名或独立季；WebDAV 结构 schema 升到 `webdav-v12` 以重建旧错误分组
...
3. 页面通过工作流阶段回调实时替换当前提示；转存落稳、可选名称修正和 SmartStrm 触发保持串行，媒体源刷新安排后使用后台 Future，不阻塞保存结果返回
