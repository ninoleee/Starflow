...
- 高性能模式开启时，Hero 会被强制收敛为静态单卡、海报样式、无阴影，并关闭翻页按钮、指示点与全屏背景图
...
- `MediaItem` 生成
- 剧集父子关系聚合
- 季目录不会只靠单个目录名直接成立；需要同级里存在同一种季命名方式的多个兄弟目录，例如 `Season 1 / Season 2`、`第1季 / 第2季`、`2.巴以 / 5.美国 / 9.韩国`
- 一旦同级确认这一层是季目录，那么上一级目录就是剧名；像 `怪奇物语/Season 1/Season 2` 会把 `怪奇物语` 当剧名
- 同级多个“同剧名 stem + 季号 + 发布信息”的发布目录，也会被当成季层，例如 `怪奇物语/Stranger.Things.S02.2160p.BluRay.REMUX`、`怪奇物语/Stranger.Things.S05.2160p.BluRay.REMUX`；这时仍保留真实季号 `2 / 5`，不会按遍历顺序重新编号
- 像 `十三邀第九季` 这类“标题本体 + 季后缀”的目录，如果上层没有形成这种同级季结构，就继续保留目录自身标题，不直接回退到集合根或来源目录名
...
- `_MpvControlsOverlay` 的自动隐藏定时器、hover 同步和 `setState` 现在都受 `_isDisposed / _canUpdateOverlayState` 保护，减少全屏切换或返回时的 `setState after dispose` 与后续 `mouse_tracker` 异常
- `lib/core/utils/playback_trace.dart` 提供 `windows-mpv.*` scoped trace，覆盖初始化、缓冲、显隐、全屏和销毁阶段，便于桌面端排障
- `TV` 分支当前仍保留自定义播放叠层：
...
- 高性能模式开启时，透明磨砂效果、自动隐藏菜单栏、Hero 样式和 Hero 全屏背景图会显示实际生效值并禁用编辑，避免出现“可改但不生效”
