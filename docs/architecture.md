...
- 如果网络存储里开启了“同步删除夸克目录”，且被删的是指向夸克网盘直链的 `WebDAV .strm` 文件，还会按当前夸克保存目录去匹配并同步删除对应影片或剧集目录
...
- 可选开启“同步删除夸克目录”，让 `WebDAV .strm` 删除成功后联动删除当前夸克保存目录里的对应影片或剧集目录
