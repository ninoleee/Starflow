...
- `MediaItem` 生成
- 剧集父子关系聚合
- 当路径里已经出现显式季目录时，例如 `Season 1 / 第1季 / 5.美国`，即使当前只有一季，也会继续保留“剧 -> 季 -> 集”层级，不再因为单季而直接拍平成集列表
- 当父目录已识别为同一部剧时，`2.巴以 / 5.美国 / 9.韩国` 这类数字子目录即使下面有多个直放文件，也会继续归到父级对应季，而不是把子目录重新起成一部新的“第一季”
...
- 非 `TV` 的内嵌 `MPV` 默认优先使用 `media_kit` 自带控制条：
  ...
  - 顶部返回按钮仍然通过 `media_kit` 控制条主题注入，但会把左侧 margin 压到更贴近原生播放器左上角的位置
