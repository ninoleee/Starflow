...
- Android 原生播放器额外包含与 Media3 同版本的 `media3-exoplayer-hls`；仅对 SmartStrm 且文件名含 `#/%23` 的原生启动执行响应头级预检，按最终 Content-Type 直接选择 MP4/HLS，不持久化短期重定向地址；标准 `/smartstrm/` 与 `/smartstrm_*/` 路径在首次解析错误 `3003` 后都会由 `NativePlaybackHlsFallbackPolicy` 保留进度并切换 HLS 一次
