...
- 电影版本目录使用保守的兄弟目录规则：同一影片根目录下至少两个技术标签目录同时存在，且所有文件都无明确季集信号时，才会被标记为同一 `movie` 的多个可播放版本；版本目录下的 `Disc / CD / Remux` 等嵌套组织层会继续沿用外层影片根目录，`S01E01` 等剧集标记和 `4K 12集` 这类剧集包装层不进入该分支
- 电影多版本会在详情页 Hero 下方提供直接可见的“播放版本”选择控件；“本地资源”仅按来源分组并负责来源切换，播放版本仅按当前来源展开，两个选择互不混用；WMDB / TMDB 匹配统一使用影片根目录名，清晰度和编码等文件名后缀只保留给版本展示，明确匹配的 NFO / sidecar 标题优先
...
- iOS 后台音频播放会话，`AppDelegate`、内嵌播放系统会话和原生 `AVPlayer` 容器共用 `StarflowAudioSession` 配置入口；配置失败会写入结构化 native 日志
...
- 后台播放
  ...
  - iOS：开启后播放中切后台时启用后台音频会话，继续后台播放音频；系统 Now Playing 信息会优先使用 `PlaybackTarget.posterUrl` 展示锁屏 / 控制中心封面，缺海报时回退到 `backdropUrl`；音频会话配置由统一 native helper 执行并记录失败原因
