...
- 用户触发的增量刷新默认保留未变化 WebDAV 子目录快照，只重新读取当前作用域根目录以发现新增或删除；索引和元数据阶段只处理新发现文件，已有记录直接复用。夸克保存后发现新文件时仍显式失效对应来源快照；如需强制重新遍历并修复旧条目，使用重建索引
...
3. 按“索引刷新等待时间”延迟执行指定媒体源的增量刷新；若保存结果包含新文件，刷新会在等待结束并停止旧任务后，先按来源删除持久化 WebDAV 目录快照，再读取真实目录，避免服务端目录指纹未变化时复用旧子树。普通增量刷新保留快照，并只为新文件建立索引和补元数据
4. 首页和媒体库读取到新的索引或缓存
...
- 三个页面都不再维护需要手动提交的页面草稿：选择、开关和步进项修改后立即排入持久化队列，文本输入使用 `250ms` 合并窗口；返回时会冲刷并等待最后一笔写入，然后直接关闭页面，不再显示保存确认框或工具栏提交按钮
