...
截至 `2026-08-23`，生产应用的首页模块加载和后台元数据预取使用两套独立预算。真机基线虽然使用固定夹具，但比较真实业务构建时仍应同时记录高性能模式、启动首页/Emby 刷新开关、首页最大并发与首批数量、元数据最大并发与首批数量，以及日志记录级别，避免把设置差异误判成代码回退。
...
- 对真实业务场景做补充测量时，固定后台元数据并发（默认 `2`）、元数据首批数量（默认 `12`）、首页模块并发（默认 `2`）、首页首批模块数（默认 `2`）和本地日志级别。
