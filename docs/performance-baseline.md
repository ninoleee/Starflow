...
截至 `2026-08-24`，生产应用的首页模块加载和后台元数据预取使用两套独立预算。真机基线虽然使用固定夹具，但比较真实业务构建时仍应同时记录独立的界面 / Hero / 播放设置、启动首页/Emby 刷新开关、首页并发/首批/批次间隔、元数据并发/首批/批次间隔/交互恢复时间、NAS 三层并发，以及日志记录级别，避免把设置差异误判成代码回退。TV 固定保护属于平台基线，不作为可调开关记录。
...
- 对真实业务场景做补充测量时，固定后台元数据并发/首批/批次间隔/交互恢复时间（默认 `2 / 12 / 300ms / 400ms`）、首页模块并发/首批/批次间隔（默认 `2 / 2 / 350ms`）、NAS 来源/集合/补全并发（默认 `1 / 2 / 2`）和本地日志级别。
...
如果要专门验证低性能 TV 的启动与首页体感，除了五个固定夹具场景，还应人工检查日志中的 `home.scheduler`、`metadata.prefetch-scheduler`、`app.frame-performance` 和 `network.http`：同一次冷启动不应出现两轮首页自动刷新，前台交互期间不应继续放大后台预取扇出，网络失败应带统一的 `failureKind`。
