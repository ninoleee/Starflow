...
void main() {
  for (final television in [false, true]) {
    for (final surface in ['page', 'alert', 'simple', 'dialog', 'sheet']) {
      testWidgets('primary style on $surface, TV: $television', (tester) async {
        ...
        expect(backgrounds(tester, find.byType(StarflowIconButton)),
            contains(expected));
        expect(
  ...
  for (final accent in AppAccent.values) {
    for (final iconOnly in [false, true]) {
      for (final surface in ['page', 'dialog']) {
        testWidgets(
            'TV focus contrast: ${accent.name}, icon=$iconOnly, $surface',
            (tester) async {
          ...
          Future<int> borderRed() async {
            ...
            return (await tester.runAsync(() async {
              ...
              try {
                ...
                return bytes.getUint8((image.width * 2 + image.width ~/ 2) * 4);
              } finally {
                image.dispose();
              }
            }))!;
          ...
          final unfocusedSize = tester.getSize(find.byKey(boundaryKey));
          focus.requestFocus();
          await tester.pumpAndSettle();
          final focusedRed = await borderRed();
          expect(focusedRed, greaterThan(230));
          expect(focusedRed - unfocusedRed, greaterThan(100));
          expect(tester.getSize(find.byKey(boundaryKey)), unfocusedSize);
          await tester.sendKeyEvent(LogicalKeyboardKey.select);
          await tester.pumpAndSettle();
          expect(presses, 1);
          focus.unfocus();
          await tester.pumpAndSettle();
          expect(await borderRed(), unfocusedRed);
